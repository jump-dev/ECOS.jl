...
function ECOSMathProgModel(; kwargs...)
    return ECOSMathProgModel(
        0,
        0,
        0,
        0,
...
function MPB.LinearQuadraticModel(s::ECOSSolver)
    return MPB.ConicToLPQPBridge(MPB.ConicModel(s))
end
...
function MPB.optimize!(m::ECOSMathProgModel)
    ...
    return m.obj_val = dot(m.c, m.primal_sol) * (m.orig_sense == :Max ? -1 : +1)
end
...
function MPB.supportedcones(m::ECOSSolver)
    return [:Free, :Zero, :NonNeg, :NonPos, :SOC, :ExpPrimal]
end
...
function MPB.loadproblem!(
    ...
    if size(A, 2) == 0
        Base.warn(
            "Input matrix has no columns. ECOS is known to crash in this corner case.",
        )
    end
    m.nconstr = size(A, 1)
    ...
    for cone_vars in constr_cones
        cone_vars[1] in bad_cones &&
            error("Cone type $(cone_vars[1]) not supported")
    end
    ...
    @assert G_row ==
            num_pos_orth + num_G_row_soc + sum(all_rows) + num_G_row_exp + 1
    ...
    m.h = ecos_h
    m.b = ecos_b
    return m.fwd_map = fwd_map           # Used to return solution
end
